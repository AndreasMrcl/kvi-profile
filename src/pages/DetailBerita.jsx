      ...
      <div className="relative bg-navy-800 pt-[88px] md:pt-[104px] overflow-hidden">
        ...
        <div className="relative max-w-[900px] mx-auto px-6 md:px-8 py-14">
          ...
          <nav className="flex items-center gap-2 mb-6 flex-wrap">
            {[
              { label: "Beranda", path: "/" },
              { label: "Berita & Publikasi", path: "/berita" },
              { label: artikel.judul },
            ].map((b, i, arr) => (
          ...
          <div className="flex items-center gap-2 mb-5 flex-wrap">
            <span
              className={`text-[11px] font-bold uppercase tracking-wide px-2.5 py-0.5 rounded ${KAT_COLOR[artikel.kategori]}`}
            >
              {artikel.kategori}
            </span>
          ...
          <h1
            className="font-display text-3xl md:text-4xl font-extrabold text-white leading-[1.18] mb-4"
            style={{ textWrap: "pretty" }}
          >
            {artikel.judul}
          </h1>
          <p className="text-white/70 text-[15px] leading-[1.75] max-w-[680px] mb-8">
            {artikel.subjudul}
          </p>
          ...
          <div className="flex items-center justify-between gap-6">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-kvi-600 flex items-center justify-center font-display font-bold text-white">
                K
              </div>
              <div>
          ...
          <div className="grid md:grid-cols-[1fr_280px] gap-12 items-start">
            ...
            <div id="article-body">
                ...
                {artikel.konten.map((b, i) => {
                  if (b.tipe === "p")
                    return (
                      <p
                        key={i}
                        className="text-[15px] text-zinc-600 leading-[1.85] mb-5"
                      >
                        {b.isi}
                      </p>
                    );
                  if (b.tipe === "h2")
                    return (
                      <h2
                        key={i}
                        className="font-display text-2xl font-bold text-navy-800 mt-10 mb-4"
                      >
                        {b.isi}
                      </h2>
                    );
                  if (b.tipe === "h3")
                    return (
                      <h3
                        key={i}
                        className="font-display text-xl font-bold text-navy-800 mt-8 mb-3"
                      >
                        {b.isi}
                      </h3>
                    );
                  if (b.tipe === "quote")
                    return (
                      <blockquote
                        key={i}
                        className="border-l-4 border-kvi-500 pl-5 py-2 bg-kvi-50 rounded-r-xl my-8 text-[15px] text-zinc-600 italic leading-relaxed"
                      >
                        {b.isi}
                      </blockquote>
                    );
                    ...
                    return (
                      <ul key={i} className="space-y-3 mb-6">
                        {b.isi.map((li, j) => (
                          <li
                            key={j}
                            className="flex items-start gap-3 text-[15px] text-zinc-600 leading-relaxed"
                          >
              ...
              <div className="flex items-center gap-2 flex-wrap mt-10 pt-8 border-t border-paper-200">
                ...
                {artikel.tags.map((t) => (
                  <button
                    key={t}
                    className="px-3 py-1 bg-paper-100 hover:bg-kvi-50 hover:text-kvi-600 text-zinc-500 rounded-lg text-xs font-semibold border border-paper-200 hover:border-kvi-100 transition-all"
                  >
                    {t}
                  </button>
                ))}
              ...
              <div className="grid sm:grid-cols-2 gap-4 mt-8">
                <Link
                  to={`/berita/${prev.slug}`}
                  className="flex items-center gap-3 p-5 bg-paper-50 rounded-xl border border-paper-200 hover:border-kvi-200 transition-all text-left"
                >
                  <svg
                    className="w-5 h-5 text-navy-800 flex-shrink-0"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={2}
                  ...
                  <div>
                    <div className="text-[10px] text-zinc-400 font-semibold uppercase tracking-widest mb-1">
                      Sebelumnya
                    </div>
                    <div className="text-sm font-bold text-navy-800 leading-snug">
                      {prev.judul}
                    </div>
                  </div>
                </Link>
                <Link
                  to={`/berita/${next.slug}`}
                  className="flex items-center justify-end gap-3 p-5 bg-paper-50 rounded-xl border border-paper-200 hover:border-kvi-200 transition-all text-right"
                >
                  <div>
                    <div className="text-[10px] text-zinc-400 font-semibold uppercase tracking-widest mb-1">
                      Berikutnya
            ...
            <div className="sticky top-24 flex flex-col gap-5">
              ...
              <div className="bg-paper-50 rounded-xl border border-paper-200 p-5">
                <div className="text-[10px] font-bold text-navy-800 uppercase tracking-widest mb-4">
                  Daftar Isi
                </div>
                {[
                  "Jadwal Pelaksanaan",
                  ...
                  <button
                    key={i}
                    className="w-full flex items-center gap-2 text-left text-sm text-zinc-500 hover:text-kvi-600 py-1.5 px-2 rounded-lg hover:bg-kvi-50 transition-all"
                  >
                    <svg
                      className="w-3.5 h-3.5 text-kvi-400 flex-shrink-0"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      strokeWidth={2}
              ...
              <div className="bg-navy-800 rounded-xl p-5 text-white">
                ...
                {artikel.infoBox.map((info, i) => (
                  <div
                    key={i}
                    className={`flex items-center justify-between py-2.5 ${i < artikel.infoBox.length - 1 ? "border-b border-white/8" : ""}`}
                  >
                    <span className="text-xs text-white/55">{info.label}</span>
                    <span className="text-xs font-bold text-kvi-300">
                      {info.val}
                    </span>
                  </div>
                ))}
                ...
                <div className="flex gap-2">
                  {[
                    ...
                    },
                    {
                      label: "Twitter",
                      color: "bg-zinc-900",
                      d: "M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.742l7.727-8.844L2.007 2.25H8.47l4.264 5.628z",
                    },
                    {
                      label: "WhatsApp",
                      color: "bg-[#25d366]",
                    ...
                    <button
                      key={s.label}
                      aria-label={s.label}
                      className={`flex-1 py-2.5 ${s.color} text-white rounded-lg flex items-center justify-center hover:opacity-90 transition-opacity`}
                    >
                      <svg
                        className="w-3.5 h-3.5"
                        fill="currentColor"
        ...
        <section className="bg-paper-100 py-16">
          <div className="max-w-[1320px] mx-auto px-6 md:px-8">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2 className="font-display text-2xl font-bold text-navy-800 mb-1.5">
                  Berita Terkait
                </h2>
              ...
              <Link
                to="/berita"
                className="text-sm font-bold text-kvi-600 hover:text-kvi-700 transition-colors inline-flex items-center gap-1.5"
              >
                Lihat Semua
                <svg
                  className="w-3.5 h-3.5"
                ...
                >
                  ...
                  <div className="p-5">
                    <div className="flex items-center gap-2 mb-2">
                      <span
                        className={`text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded ${KAT_COLOR[b.kategori] || ""}`}
                      >
                        {b.kategori}
                      </span>
                    ...
                    <h3 className="font-display font-bold text-navy-800 text-[15px] leading-snug">
                      {b.judul}
                    </h3>
                  </div>
                </Link>
