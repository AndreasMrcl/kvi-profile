...
const TIPE_COLOR = {
  Peraturan: {
    pill: "bg-kvi-50 text-kvi-700 border-kvi-100",
    icon: "text-kvi-600",
  },
  Kebijakan: {
    pill: "bg-navy-50 text-navy-700 border-navy-100",
    icon: "text-navy-700",
  },
  Panduan: {
    ...
    <div className="grid sm:grid-cols-2 rounded-2xl overflow-hidden bg-white border border-zinc-100 shadow-card mb-10 reveal-item">
      ...
      <div className="p-8 md:p-10 flex flex-col justify-center">
        <div className="flex items-center gap-2 mb-4">
          <span
            className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${c.pill}`}
          >
            {item.kategori}
          </span>
        ...
        <h2
          className="font-display text-2xl font-bold text-navy-800 leading-snug mb-4"
    ...
    >
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex items-center gap-2 text-[10px] uppercase tracking-widest mb-2">
          <span className={`font-semibold px-2 py-0.5 rounded ${c.pill}`}>
            {item.kategori}
          </span>
          <span className="text-zinc-400">{item.tanggal}</span>
        </div>
        <h3 className="font-display font-bold text-navy-800 text-[15px] leading-snug mb-2 line-clamp-2 flex-1">
          {item.judul}
        </h3>
  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        <div
          className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 border ${c.pill}`}
        >
          <svg
        ...
        <div className="flex-1 min-w-0">
          ...
          <h3 className="font-display font-bold text-navy-800 text-[14px] leading-snug line-clamp-2">
            {item.judul}
          </h3>
        </div>
      </div>
...
function EventsSidebar() {
  return (
    <div className="flex flex-col gap-5">
      {/* Acara */}
      <div>
        <h2 className="font-display text-xl font-bold text-navy-800 mb-1.5">
          ACARA MENDATANG
        </h2>
      ...
      <Link
        to="/registrasi"
        className="group relative block bg-gradient-to-br from-kvi-600 to-kvi-800 rounded-xl p-6 text-white overflow-hidden shadow-card hover:shadow-xl transition-shadow"
      >
...
function BeritaTab() {
  ...
  const filtered = berita.filter((b) => {
    ...
    const matchQ =
      !query || b.judul.toLowerCase().includes(query.toLowerCase());
      ...
      {total > 1 && (
        <div className="flex items-center justify-center gap-2">
          <button
            ...
            disabled={page === 1}
            className="w-9 h-9 rounded-lg border border-zinc-200 bg-white flex items-center justify-center disabled:opacity-40"
          >
            <svg
...
function PublikasiTab() {
  ...
  const filtered = publikasi.filter((p) => {
    ...
    const matchQ =
      !query || p.judul.toLowerCase().includes(query.toLowerCase());
  ...
  return (
    <div>
        ...
        <div className="relative flex items-center justify-between gap-6">
          <div>
            ...
            <p className="text-white/65 text-sm max-w-md">
              Peraturan, kebijakan, dan panduan resmi yang diterbitkan oleh
          ...
          <div className="hidden md:flex gap-8 flex-shrink-0">
            {[
              ...
              <div key={l} className="text-center">
                ...
                <div className="text-[10px] text-white/55 font-bold uppercase tracking-wider mt-1">
                  {l}
                </div>
              </div>
            ))}
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        {filtered.map((item, i) => (
          <PublikasiCard key={item.id} item={item} delay={i * 60} />
        ))}
      </div>
      {filtered.length === 0 && (
        <div className="text-center py-16 text-zinc-400">
